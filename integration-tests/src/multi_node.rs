            ...
            .enumerate()
        {
            ...
            let node = Tester::launch_node_without_wait_with_network_port(
                ...
                Some(move |config: &mut Config| {
                    ...
                    // Use short timeouts for tests so leader election completes in <1s.
                    config.consensus_config.election_timeout_min = Duration::from_millis(250);
                    config.consensus_config.election_timeout_max = Duration::from_millis(750);
                    config.consensus_config.heartbeat_interval = Duration::from_millis(100);
                }),
                ChainLayout::Default {
                    protocol_version: PROTOCOL_VERSION,
                },
