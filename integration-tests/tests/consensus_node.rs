...
async fn consensus_cluster_rotates_leader_after_failure() -> anyhow::Result<()> {
    ...
    let result = async {
        ...
        deploy_event_emitter_and_wait_for_active_replication(&cluster, initial_leader_idx).await?;
...
async fn consensus_original_leader_rejoins_and_cluster_remains_stable() -> anyhow::Result<()> {
    ...
    let result = async {
        ...
        deploy_event_emitter_and_wait_for_active_replication(&cluster, final_leader_idx).await?;
