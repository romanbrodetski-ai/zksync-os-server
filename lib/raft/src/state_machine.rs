...
pub struct RaftStateMachineStore {
    ...
    /// Read-only handle to the WAL. Used by `applied_state()` to derive
    /// `last_applied_log_id` from the WAL's latest committed block number.
            ...
            for entry in &entries {
                match &entry.payload {
                    ...
                    EntryPayload::Membership(membership) => {
                        ...
                        tracing::debug!(log_id = ?entry.log_id, "membership change persisted");
                        responses.push(());
                    }
                }
            }
    ...
    fn log_pending_applied_blocks(
        ...
        if !pending.is_empty() {
            tracing::info!(
                ...
                — OpenRaft will re-apply the missing entries",
                pending.len(),
            );
        }
        Ok(())
