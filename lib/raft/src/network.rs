...
impl RaftRequestHandler for RaftRpcHandler {
    async fn handle(&self, request: RaftRequest) -> Result<RaftResponse, String> {
        ...
        tracing::debug!("handling incoming raft rpc request ({request_kind})");
        match request {
...
impl RaftNetwork<RaftTypeConfig> for RaftNetworkClient {
    ...
    > + Send {
        ...
        async move {
            ...
            match resp_result {
                ...
                Ok(other) => unreachable!("append_entries rpc returned wrong response variant: {other:?}"),
    ...
    > + Send {
        ...
        async move {
            tracing::debug!(
                "sending raft vote rpc to {peer_id:?} for leader {:?} (timeout_ms={})",
                rpc.vote.leader_id,
            ...
            match resp_result {
                ...
                Ok(other) => unreachable!("vote rpc returned wrong response variant: {other:?}"),
                Err(err) => Err(rpc_unreachable(err)),
            }
        }
    }
    ...
    > + Send {
        ...
        async move {
            tracing::debug!(
                "sending raft install_snapshot rpc to {peer_id} \
                 (timeout_ms={}, hard_ttl_ms={}, client_timeout_ms={})",
                timeout_dur.as_millis(),
                option.hard_ttl().as_millis(),
            ...
            match resp_result {
                ...
                Ok(other) => unreachable!("install_snapshot rpc returned wrong response variant: {other:?}"),
