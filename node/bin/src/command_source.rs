...
impl ConsensusRole {
    fn from_is_leader(is_leader: bool) -> Self {
        if is_leader {
            Self::Leader
        } else {
            Self::Replica
...
#[async_trait]
trait LeadershipSignalExt {
    fn current_role(&self) -> ConsensusRole;
    async fn wait_for_change(&mut self) -> Result<(), tokio::sync::watch::error::RecvError>;
}

#[async_trait]
impl LeadershipSignalExt for LeadershipSignal {
    fn current_role(&self) -> ConsensusRole {
        match self {
            Self::AlwaysLeader => ConsensusRole::Leader,
            Self::Watch(rx) => ConsensusRole::from_is_leader(*rx.borrow()),
        }
    }
    ...
    async fn wait_for_change(&mut self) -> Result<(), tokio::sync::watch::error::RecvError> {
        match self {
            Self::AlwaysLeader => {
                future::pending::<Result<(), tokio::sync::watch::error::RecvError>>().await
            }
